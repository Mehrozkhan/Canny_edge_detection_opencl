...
void gaussianfilter(const std::vector<float>& h_input, std::vector<float>& h_outputCpu, std::size_t countX, std::size_t countY)
{
	float weights[5][5] = {
		{1,  4,  7,  4, 1},
		{4, 16, 26, 16, 4},
...
void sobelHost(const std::vector<float>& h_input1, std::vector<float>& h_outputCpu1, std::vector<int>& h_out_segment, std::size_t countX, std::size_t countY) {
	for (int i = 0; i < (int) countX; i++) {
		for (int j = 0; j < (int) countY; j++) {
			float Gx = getValueGlobal(h_input1, countX, countY, i-1, j-1)+2*getValueGlobal(h_input1, countX, countY, i-1, j)+getValueGlobal(h_input1, countX, countY, i-1, j+1)
					-getValueGlobal(h_input1, countX, countY, i+1, j-1)-2*getValueGlobal(h_input1, countX, countY, i+1, j)-getValueGlobal(h_input1, countX, countY, i+1, j+1);
			float Gy = getValueGlobal(h_input1, countX, countY, i-1, j-1)+2*getValueGlobal(h_input1, countX, countY, i, j-1)+getValueGlobal(h_input1, countX, countY, i+1, j-1)
					-getValueGlobal(h_input1, countX, countY, i-1, j+1)-2*getValueGlobal(h_input1, countX, countY, i, j+1)-getValueGlobal(h_input1, countX, countY, i+1, j+1);
			h_outputCpu1[getIndexGlobal(countX, i, j)] = sqrt(Gx * Gx + Gy * Gy);
...
void non_max_suppression(const std::vector<float>& h_input2, std::vector<float>& h_outputCpu2, std::vector<int>& h_in_segment1, std::size_t countX, std::size_t countY)
{
	for (int i = 0; i < (int)countX; i++) {
		for (int j = 0; j < (int)countY; j++)
		{
			switch (h_in_segment1[getIndexGlobal(countX, i, j)]) {
			case 1:
				if (h_input2[getIndexGlobal(countX, i, j) - 1] >= h_input2[getIndexGlobal(countX, i, j)] || h_input2[getIndexGlobal(countX, i, j) + 1] > h_input2[getIndexGlobal(countX, i, j)])
					h_outputCpu2[getIndexGlobal(countX, i, j)] = 0;
				else h_outputCpu2[getIndexGlobal(countX, i, j)] = h_input2[getIndexGlobal(countX, i, j)];
				break;
			case 2:
				if (h_input2[getIndexGlobal(countX, i, j) - (countX - 1)] >= h_input2[getIndexGlobal(countX, i, j)] || h_input2[getIndexGlobal(countX, i, j) + (countX - 1)] > h_input2[getIndexGlobal(countX, i, j)])
					h_outputCpu2[getIndexGlobal(countX, i, j)] = 0;
				else h_outputCpu2[getIndexGlobal(countX, i, j)] = h_input2[getIndexGlobal(countX, i, j)];
				break;
			case 3:
				if (h_input2[getIndexGlobal(countX, i, j) - (countX)] >= h_input2[getIndexGlobal(countX, i, j)] || h_input2[getIndexGlobal(countX, i, j) + (countX)] > h_input2[getIndexGlobal(countX, i, j)])
					h_outputCpu2[getIndexGlobal(countX, i, j)] = 0;
				else h_outputCpu2[getIndexGlobal(countX, i, j)] = h_input2[getIndexGlobal(countX, i, j)];
				break;
			case 4:
...
int main(int argc, char** argv) {
	...
	cl::Program program = OpenCL::loadProgramSource(context, "E:/GPU_lab/Exercise_1/Exercise_1/Exercise 1/For Windows/Opencl-Basics-Windows/Opencl-ex1/src/OpenCLExercise3_Sobel.cl");
	...
	std::vector<float> h_outputCpu1(count);
	std::vector<int> h_out_segment(count);
	std::vector<float> h_input2(count);
	std::vector<float> h_outputCpu2(count);
	std::vector<int> h_in_segment1(count);
	...
	*/
	// Use an image (Valve.pgm) as input data
	{
		...
		Core::readImagePGM("E:/GPU_lab/Exercise_1/Exercise_1/Exercise 1/For Windows/Opencl-Basics-Windows/Opencl-ex1/src/Valve.pgm", inputData, inputWidth, inputHeight);
	...
	gaussianfilter(h_input, h_outputCpu, countX, countY);
	sobelHost(h_outputCpu, h_outputCpu1, h_out_segment, countX, countY);
	non_max_suppression(h_outputCpu1, h_outputCpu2, h_out_segment, countX, countY);
	Core::TimeSpan cpuend = Core::getCurrentTime();
	//////// Store CPU output image ///////////////////////////////////
	Core::writeImagePGM("E:/GPU_lab/project/output_gaussianfilter_cpu.pgm", h_outputCpu, countX, countY);
	Core::writeImagePGM("E:/GPU_lab/project/output_sobel_cpu.pgm", h_outputCpu1, countX, countY);
	Core::writeImagePGM("E:/GPU_lab/project/output_non_max_suppression.pgm", h_outputCpu2, countX, countY);
	std::cout << std::endl;
