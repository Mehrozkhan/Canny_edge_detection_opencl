...
float low_threshold1 = 0.58;
float high_threshold1 = 1.48;
	...
	for (int i = 0; i < (int)countX; i++) {
		for (int j = 0; j < (int)countY; j++) {
			...
			if (h_input[getIndexGlobal(countX, i, j)] > strong_threshold)
				h_outputCpu[getIndexGlobal(countX, i, j)] = /*1.0; */ 255;   //absolutely edge
			else if (h_input[getIndexGlobal(countX, i, j)] > weak_threshold)
			{
				h_outputCpu[getIndexGlobal(countX, i, j)] = /*0.5; */128;   //potential edge
...
void apply_edge_hysteresis(std::vector<float>& h_outputCpu, const std::vector<float>& h_input, std::size_t countX, std::size_t countY) {
	...
	for (int i = 1; i < countX - 1; i++) {
		for (int j = 1; j < countY - 1; j++) {
			...
			if (h_input[ getIndexGlobal(countX, i, j) ] ==/* 0.5)*/128 )
			{
...
int main(int argc, char** argv) {
	...
	cl::Program program = OpenCL::loadProgramSource(context, "E:/GPU_lab/Exercise_1/Exercise_1/Exercise 1/For Windows/Opencl-Basics-Windows/Opencl-ex1/src/OpenCLExercise3_Sobel_DobleTh.cl");
	...
	cl_float low_threshold = 0.58;
	cl_float high_threshold = 1.48;
	...
	apply_double_threshold(h_outputCpu_DoubleThreshold, h_outputCpu_NonMaxSupression, high_threshold1, low_threshold1, countX, countY);
	...
	Core::writeImagePGM("E:/GPU_lab/project/output_gaussianfilter_cpu.pgm", h_outputCpu_Gaussian, countX, countY);
	Core::writeImagePGM("E:/GPU_lab/project/output_sobel_cpu.pgm", h_outputCpu_Sobel, countX, countY);
	Core::writeImagePGM("E:/GPU_lab/project/output_non_max_suppression.pgm", h_outputCpu_NonMaxSupression, countX, countY);
	Core::writeImagePGM("E:/GPU_lab/project/output_DoubleThreshold.pgm", h_outputCpu_DoubleThreshold, countX, countY);
	Core::writeImagePGM("E:/GPU_lab/project/output_Hysterisis.pgm", h_outputCpu_Hysteresis, countX, countY);
	std::cout << std::endl;
	...
	//TODO
	cl::Event eventS1;
	queue.enqueueWriteBuffer(d_input, true, 0, size, h_outputCpu_Gaussian.data(), NULL, &eventS1);
	...
	cl::Event eventS3;
	queue.enqueueReadBuffer(d_output, true, 0, size, h_outputGpu.data(), NULL, &eventS3);
	cl::Event eventS4;
	...
	cl::Event eventDt2;
	queue.enqueueNDRangeKernel(DoubleThresholdKernel,
		cl::NullRange,
		cl::NDRange(countX, countY),
		cl::NDRange(wgSizeX, wgSizeY),
		NULL,
	...
	cl::Event eventHst2;
	queue.enqueueNDRangeKernel(HysteresisKernel,
		cl::NullRange,
		cl::NDRange(countX, countY),
		cl::NDRange(wgSizeX, wgSizeY),
		NULL,
	...
	/*Core::TimeSpan gputimesobel1 = OpenCL::getElapsedTime(event2);

	Core::TimeSpan gputimesobel2 = OpenCL::getElapsedTime(event3);

	Core::TimeSpan totalgputimesobel = gputimesobel1 + gputimesobel2;
	std::cout << "GPU time before copying output data: " << gputime1.toString() << " speedup before copy = " << (cputimeSobel.getSeconds() / gputimesobel1.getSeconds()) << std::endl;
	std::cout << "GPU time after copying output data: " << totalgputime.toString() << " speedup after copy = " << (cputimeSobel.getSeconds() / totalgputimesobel.getSeconds()) << std::endl;
	*/
	//////// Store GPU output image ///////////////////////////////////
	//Core::writeImagePGM("output_sobel_gpu.pgm", h_outputGpu, countX, countY);
	Core::writeImagePGM("E:/GPU_lab/project/output_DoubleThresholdGPU.pgm", h_outputGpuDoublethreshold, countX, countY);
	Core::writeImagePGM("E:/GPU_lab/project/output_HysteresisGPU.pgm", h_outputGpu_Hysteresis, countX, countY);
